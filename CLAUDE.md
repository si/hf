...
- Cover art (`coverImage` front matter, e.g. `HFxxx_with_DJName.jpg`) usually can't be added directly — artwork typically arrives as a pasted image in chat, not a file on disk. When that happens:
  ...
  - Si often can't rename the file extension on upload (e.g. uploads `.jpeg` when the post references `.jpg`). After he confirms the upload, pull the branch and check the actual filename committed — update `coverImage` in the post's front matter to match exactly, don't assume the extension.
